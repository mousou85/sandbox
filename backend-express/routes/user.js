...
module.exports = (db) => {
   ...
   * 회원 정보 수정
   */
  router.post('/info', authTokenMiddleWare, asyncHandler(async (req, res) => {
    //set vars: user idx
    const userIdx = req.user.user_idx;
    ...
    if (!name && (!currentPassword || !newPassword)) throw new ResponseError('수정할 정보 입력 필요');
    
    //set vars: user data
    let rsUser = await db.queryRow(db.queryBuilder()
      .select(['user_idx', 'id', 'password', 'name'])
      .from('users')
      .where('user_idx', userIdx)
    );
    if (!rsUser) throw new ResponseError('잘못된 접근임');
    
    //current password check
    if (currentPassword) {
      //set vars: password salt
      let passwordSalt = await db.queryScalar(db.queryBuilder()
        .select('salt')
        .from('users_password_salt')
        .where('user_idx', userIdx)
      );
      if (!passwordSalt) throw new ResponseError('패스워드 오류');
      
      if (!userHelper.verifyPassword(currentPassword, passwordSalt, rsUser.password)) {
        throw new ResponseError('패스워드 검증 실패');
      }
    }
    ...
    try {
      //set vars: update fields
      let updateData = {};
      let updatePasswordSalt = '';
      
      if (name) updateData.name = name;
      if (currentPassword && newPassword) {
        const passwordInfo = userHelper.encryptPassword(newPassword);
        updateData.password = passwordInfo.hashedPassword;
        updatePasswordSalt = passwordInfo.salt;
      }
      ...
      if (updateData.password && updatePasswordSalt) {
        await db.execute(db.queryBuilder()
          .update({salt: updatePasswordSalt})
          .from('users_password_salt')
          .where('user_idx', userIdx)
        , dbTrx);
      }
