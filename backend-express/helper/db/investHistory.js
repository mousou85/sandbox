...
module.exports = (db) => {
   ...
   * @return {Promise<void>}
   */
  const upsertMonthSummary = async (itemIdx, date, trx) => {
    //set vars: 날짜 관련
    ...
    //set vars: 단위 목록
    const rsUnitList = await db.queryAll(db.queryBuilder()
      .select('unit_idx')
    ...
    try {
      for (const unit of rsUnitList) {
        ...
        if (await db.exists(query, trx ?? null)) {
          ...
          upsertData['unit_idx'] = unit.unit_idx;
   ...
   * @return {Promise<void>}
   */
  const upsertYearSummary = async (itemIdx, date, trx) => {
    //set vars: 날짜 관련
    const targetDate = date ? dayjs(date) : dayjs();
   ...
   * @return {Promise<void>}
   */
  const upsertTotalSummary = async (itemIdx, trx) => {
    //set vars: 단위 목록
    const rsUnitList = await db.queryAll(db.queryBuilder()
      .select('unit_idx')
      .from('invest_unit_set')
